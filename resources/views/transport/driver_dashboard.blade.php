...
<body class="bg-orange-50/20 text-slate-800 font-sans min-h-screen flex flex-col transition-colors duration-300">
  <!-- Top Header -->
  <header class="bg-white/95 backdrop-blur-md border-b border-orange-100 px-6 py-4 flex items-center justify-between sticky top-0 z-30 shadow-sm">
    <div class="flex items-center gap-2">
      <div class="w-8 h-8 bg-orange-50 border border-orange-100 rounded-lg flex items-center justify-center text-orange-600">
    ...
    <!-- Dropdown Menu (2) -->
    <div class="relative">
      <button id="btn-dropdown" onclick="toggleDropdown()" class="flex items-center gap-1.5 px-3 py-1.5 bg-orange-50 hover:bg-orange-100/80 text-orange-600 border border-orange-100 rounded-lg text-xs font-bold transition">
      ...
      <!-- Dropdown Card -->
      <div id="header-dropdown" class="hidden absolute right-0 mt-2 w-48 bg-white border border-orange-100 rounded-xl shadow-2xl py-1.5 z-40 animate-in fade-in slide-in-from-top-2 duration-200">
        <button onclick="scrollToSection('profile-card')" class="w-full flex items-center gap-2 px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-orange-50 hover:text-orange-700 transition text-left border-none outline-none cursor-pointer">
          <i data-lucide="user" class="w-3.5 h-3.5"></i>
          <span>Mon Profil</span>
        </button>
        <button onclick="scrollToSection('active-courses')" class="w-full flex items-center gap-2 px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-orange-50 hover:text-orange-700 transition text-left border-none outline-none cursor-pointer">
          <i data-lucide="route" class="w-3.5 h-3.5"></i>
          <span>Courses Actives</span>
        </button>
        <a href="{{ route('driver.history') }}" class="w-full flex items-center gap-2 px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-orange-50 hover:text-orange-700 transition text-left border-none outline-none cursor-pointer decoration-none">
          <i data-lucide="history" class="w-3.5 h-3.5"></i>
          <span>Historique des courses</span>
        </a>
        <div class="border-t border-orange-50 my-1.5"></div>
  ...
  <main class="flex-grow p-4 max-w-lg mx-auto w-full space-y-6">
    <!-- Profile Card (3) -->
    <div id="profile-card" class="bg-white border border-orange-100 rounded-3xl p-5 relative overflow-hidden shadow-sm">
      <div class="absolute top-0 right-0 w-24 h-24 bg-orange-500/[0.03] rounded-bl-[4rem]"></div>
      <div class="flex items-start justify-between gap-4">
        <div class="flex items-center gap-4">
          <div class="w-16 h-16 bg-orange-50 border border-orange-100 rounded-2xl overflow-hidden flex-shrink-0 flex items-center justify-center shadow-inner">
            @if($driver->photo)
              <img src="{{ $driver->photo }}" class="w-full h-full object-cover">
            @else
              <span class="text-xl font-extrabold uppercase text-orange-400">{{ substr($driver->prenom,0,1) }}{{ substr($driver->nom,0,1) }}</span>
            @endif
          </div>
          <div>
            <div class="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-0.5">Chauffeur</div>
            <div class="text-lg font-extrabold text-slate-900">{{ $driver->fullname }}</div>
            <div class="text-xs text-orange-600 font-bold mt-1.5 flex items-center gap-1 bg-orange-50 border border-orange-100/50 rounded-lg px-2.5 py-1 w-fit">
      ...
      @endphp
      <div class="mt-4 p-4 bg-emerald-50/50 border border-emerald-100 rounded-2xl flex items-center justify-between shadow-sm">
        <div class="flex items-center gap-2.5">
          <div class="w-8 h-8 rounded-lg bg-emerald-100 border border-emerald-200/50 flex items-center justify-center text-emerald-600">
      ...
      <div class="border-t border-orange-50 mt-4 pt-3 flex items-center justify-between text-xs text-slate-500 font-semibold">
    ...
    <div id="active-courses" class="space-y-3">
      <h2 class="text-xs font-bold uppercase text-slate-400 tracking-widest flex items-center gap-2">
        <i data-lucide="route" class="w-4 h-4 text-orange-500"></i>
        <span>Courses Actives ({{ $activeReservations->count() }})</span>
      </h2>

      @forelse($activeReservations as $res)
        <a href="{{ route('driver.show', $res->driver_token) }}" class="block bg-white border border-orange-100 hover:border-orange-400 rounded-2xl p-4 transition-all hover:scale-[1.01] hover:shadow-xl hover:shadow-orange-500/5 group shadow-sm">
          <div class="flex items-center justify-between gap-3 mb-3">
            ...
            @php
              ...
              $statusColors = [
                ...
                'chauffeur_en_route'  => 'text-orange-700 bg-orange-50 border-orange-200',
          ...
          <div class="space-y-2 text-xs border-b border-orange-50 pb-3 mb-3">
          ...
          <div class="flex items-center justify-between text-xs font-semibold">
            ...
            <div class="flex items-center gap-1 font-bold text-orange-600 group-hover:text-orange-700 uppercase tracking-wider text-[10px] transition-colors">
              <span>Démarrer</span>
    ...
    <div id="completed-courses" class="space-y-3">
      <h2 class="text-xs font-bold uppercase text-slate-400 tracking-widest flex items-center gap-2">
        ...
        <span>Courses Récentes Terminées</span>
      </h2>

      <div class="bg-white border border-orange-100 rounded-2xl overflow-hidden shadow-sm font-semibold">
        <div class="divide-y divide-orange-50">
          @foreach($completedReservations->take(5) as $res)
        ...
        <div class="bg-orange-50/20 px-4 py-3 text-center border-t border-orange-50">
          <a href="{{ route('driver.history') }}" class="inline-flex items-center gap-1.5 text-xs font-bold text-orange-600 hover:text-orange-700 transition">
