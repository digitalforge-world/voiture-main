 ...
 <div class="flex h-screen bg-white dark:bg-slate-950">
  ...
  <aside id="sidebar" class="fixed inset-y-0 left-0 z-50 w-72 transition-transform duration-300 transform -translate-x-full lg:translate-x-0 lg:static lg:inset-0 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 flex flex-col">
     ...
     <div class="flex items-center">
      ...
      @if($displayMode === 'text' || $displayMode === 'both')
       <div class="flex flex-col">
        @if($isDefaultName)
         <div class="flex items-center text-sm tracking-normal leading-none">
          <span class="font-semibold text-slate-900 dark:text-white transition-colors">AUTO</span>
          <span class="font-light text-amber-500">IMPORT</span>
         </div>
        @else
         <span class="text-[11px] font-semibold tracking-wide text-slate-900 dark:text-white uppercase leading-none transition-colors">{{ $siteName }}</span>
        @endif
        <span class="text-[7px] font-semibold tracking-[0.3em] text-slate-500 uppercase mt-1">Admin Panel</span>
       </div>
      @endif
     </div>
    </a>
   ...
   <div class="flex-grow overflow-y-auto px-6 py-4 custom-scrollbar">
    <!-- Group 1: Overview -->
    <div class="mb-6">
     <h4 class="px-4 text-[10px] font-semibold tracking-[0.2em] uppercase text-slate-600 mb-4">Vue d'ensemble</h4>
     <nav class="space-y-1">
    ...
    <div class="mb-6">
     <h4 class="px-4 text-[10px] font-semibold tracking-[0.2em] uppercase text-slate-600 mb-4 transition-colors">Ventes & Opérations</h4>
     <nav class="space-y-1">
      ...
      <a href="{{ route('admin.drivers.index') }}" class="flex items-center gap-4 px-4 py-3 text-sm font-bold rounded-2xl transition {{ Request::is('admin/drivers*') ? 'bg-amber-500 text-slate-950 shadow-lg shadow-amber-900/20' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-900 hover:text-slate-900 dark:hover:text-white' }}">
       <i data-lucide="users" class="w-5 h-5"></i> Équipe Chauffeurs
      </a>
    ...
    <!-- Group 3: Catalogue -->
    <div class="mb-6">
     <h4 class="px-4 text-[10px] font-semibold tracking-[0.2em] uppercase text-slate-600 mb-4 transition-colors">Catalogue & Stock</h4>
     <nav class="space-y-1">
    ...
    <div class="mb-6">
     <h4 class="px-4 text-[10px] font-semibold tracking-[0.2em] uppercase text-slate-600 mb-4 transition-colors">Finance & Utilisateurs</h4>
     <nav class="space-y-1">
    ...
    <div class="mb-6">
     <h4 class="px-4 text-[10px] font-semibold tracking-[0.2em] uppercase text-slate-600 mb-4 transition-colors">Configuration</h4>
     <nav class="space-y-1">
    ...
    <div class="mb-6">
     <h4 class="px-4 text-[10px] font-semibold tracking-[0.2em] uppercase text-slate-600 mb-4 transition-colors">Croissance & Support</h4>
     <nav class="space-y-1">
  ...
  <div class="flex-grow flex flex-col min-w-0 overflow-hidden relative">
    ...
    <div class="flex items-center gap-6">
     ...
     <div class="flex items-center gap-4 pl-6 border-l border-slate-200 dark:border-slate-900">
      <div class="text-right hidden sm:block">
       <div class="text-sm font-semibold text-slate-900 dark:text-white tracking-normal leading-none uppercase transition-colors">{{ Auth::user()->prenom }} {{ Auth::user()->nom }}</div>
       <div class="text-[10px] font-semibold text-amber-500/80 uppercase tracking-wide mt-1 leading-none">Administrateur</div>
      </div>
   ...
   <!-- Page Content -->
   <main class="flex-grow overflow-y-auto p-6 lg:p-6 custom-scrollbar">
    ...
    <div id="notificationPortal" class="fixed inset-0 z-[200] hidden items-center justify-center p-4">
     ...
     <div class="relative bg-slate-900 border border-white/10 w-full max-w-sm overflow-hidden shadow-[0_0_50px_-12px_rgba(0,0,0,0.5)] rounded-[3rem] animate-in fade-in zoom-in duration-300">
      ...
      <div class="p-10 text-center">
       ...
       <h3 id="notif_title" class="text-xl font-semibold text-white uppercase tracking-normal mb-2"></h3>
       <p id="notif_message" class="text-slate-400 font-bold text-[10px] uppercase tracking-wide leading-relaxed"></p>
       
       <button onclick="closeNotification()" class="mt-8 w-full py-4 bg-white/5 hover:bg-white/10 text-white text-[10px] font-semibold uppercase tracking-[0.2em] rounded-2xl transition border border-white/5">Fermer</button>
      </div>
     </div>
    </div>
    ...
    <div id="confirmDeleteModal" class="fixed inset-0 z-[200] hidden items-center justify-center p-4">
     ...
     <div class="relative bg-slate-900 border border-rose-500/20 w-full max-w-sm overflow-hidden shadow-[0_0_50px_-12px_rgba(225,29,72,0.2)] rounded-[3rem] animate-in fade-in zoom-in duration-300">
      ...
      <div class="p-10 text-center">
       ...
       <h3 class="text-xl font-semibold text-white uppercase tracking-normal mb-2">Confirmation Critique</h3>
       <p id="delete_modal_message" class="text-slate-400 font-bold text-[10px] uppercase tracking-wide leading-relaxed">Cette action est irréversible. Voulez-vous vraiment supprimer cet élément ?</p>
       
       <form id="globalDeleteForm"method="POST" class="mt-8 flex gap-4">
        @csrf
        @method('DELETE')
        <button type="button"onclick="closeDeleteModal()" class="flex-1 py-4 bg-white/5 hover:bg-white/10 text-white text-[10px] font-semibold uppercase tracking-wide rounded-2xl transition border border-white/5">Annuler</button>
        <button type="submit" class="flex-1 py-4 bg-rose-600 hover:bg-rose-500 text-white text-[10px] font-semibold uppercase tracking-wide rounded-2xl transition shadow-xl shadow-rose-900/20">Supprimer</button>
       </form>
      </div>
     </div>
    </div>
