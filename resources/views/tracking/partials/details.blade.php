<div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-xl dark:shadow-2xl transition-colors">
    ...
    <div class="px-6 md:px-8 py-6 bg-slate-50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-800 transition-colors">
        <div class="flex flex-col gap-4 w-full">
            <div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800/80 pb-3">
                <h3 class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Statut du Paiement</h3>
                @if($isPaid)
                    <div class="bg-emerald-500/10 text-emerald-600 dark:text-emerald-500 px-3 py-1 rounded-lg flex items-center gap-1.5 border border-emerald-500/20 text-xs">
                        <i data-lucide="check-circle-2" class="w-3.5 h-3.5"></i>
                        <span class="font-bold">Payé en totalité</span>
                    </div>
                @elseif($resteAPayer > 0)
                    <div class="bg-amber-500/10 text-amber-600 dark:text-amber-500 px-3 py-1 rounded-lg flex items-center gap-1.5 border border-amber-500/20 text-xs">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div class="p-3 bg-white dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800">
                    <div class="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-0.5">Montant Total</div>
                    <div class="font-black text-sm text-slate-900 dark:text-white">{{ number_format($montantTotal, 0, ',', ' ') }} FCFA</div>
                </div>
                <div class="p-3 bg-white dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800">
                    <div class="text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-0.5">Déjà Payé</div>
                    <div class="font-black text-sm text-emerald-600 dark:text-emerald-500">{{ number_format($montantPaye, 0, ',', ' ') }} FCFA</div>
                </div>
                <div class="p-3 bg-white dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-950/20 dark:to-orange-950/20">
                    <div class="text-[9px] font-bold text-amber-600 dark:text-amber-500 uppercase tracking-wider mb-0.5">Reste à Payer</div>
                    <div class="font-black text-sm text-amber-700 dark:text-amber-500">{{ number_format($resteAPayer, 0, ',', ' ') }} FCFA</div>
                </div>
            </div>

            @if($resteAPayer > 0)
            <div class="pt-1">
                <div class="px-4 py-2.5 bg-slate-100 dark:bg-slate-800/80 text-slate-500 dark:text-slate-400 rounded-xl flex items-center justify-center gap-2 text-xs">
                    <i data-lucide="store" class="w-4 h-4"></i>
                    <span class="font-semibold text-[11px]">Le paiement du solde s'effectue directement en agence</span>
                </div>
            </div>
            @endif
        </div>
