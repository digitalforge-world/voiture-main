...
<div class="bg-slate-950 min-h-screen">
    <!-- Header Section -->
    <div class="relative py-20 overflow-hidden bg-slate-900/50">
        ...
        <div class="container relative px-4 mx-auto lg:px-8">
            <div class="max-w-3xl">
                ...
                <h1 class="text-4xl font-black text-white lg:text-5xl">Trouvez Votre <span class="text-amber-500">Pépite</span>.</h1>
                <p class="mt-4 text-lg text-slate-400">Explorez notre sélection de véhicules premium disponibles à l'importation directe.</p>
            </div>
        </div>
    </div>
        ...
        <div class="flex flex-col gap-12 lg:flex-row">
            ...
            <aside class="w-full lg:w-80 shrink-0">
                <form action="{{ route('cars.index') }}" method="GET" class="sticky top-28 space-y-8">
                    <div class="p-8 border bg-slate-900/50 border-slate-800 rounded-[2rem]">
                        <h3 class="flex items-center gap-2 mb-6 text-lg font-bold text-white uppercase tracking-tighter">
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative">
                                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Modèle, marque..." class="w-full py-3 pl-10 pr-4 text-sm bg-slate-950 border border-slate-800 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition text-white placeholder:text-slate-600">
                                    <i data-lucide="search" class="absolute left-3.5 top-3.5 w-4 h-4 text-slate-600"></i>
                                </div>
                            </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <select name="marque" class="w-full py-3 px-4 text-sm bg-slate-950 border border-slate-800 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition text-white">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="flex items-center gap-3">
                                    <div class="relative flex-grow">
                                        <input type="number" name="prix_max" value="{{ request('prix_max') }}" placeholder="Max" class="w-full py-3 px-4 text-sm bg-slate-950 border border-slate-800 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition text-white placeholder:text-slate-600">
                            ...
                            @if(request()->anyFilled(['search', 'marque', 'prix_max']))
                                <a href="{{ route('cars.index') }}" class="block text-center text-xs font-bold text-slate-500 hover:text-white transition"> Réinitialiser </a>
                            @endif
                        </div>
                    </div>
                </form>
            ...
            <div class="flex-grow">
                <div class="flex items-center justify-between mb-8">
                    <div class="text-sm font-medium text-slate-400">
                        <span class="text-white font-bold">{{ $voitures->count() }}</span> véhicules trouvés
                    </div>
                ...
                <div class="grid grid-cols-1 gap-8 md:grid-cols-2">
                    @forelse($voitures as $car)
                        <div class="group bg-slate-950 border border-slate-800 rounded-[2rem] overflow-hidden hover:border-amber-500/30 transition duration-500 flex flex-col shadow-xl">
                            ...
                            <div class="p-8 flex-grow flex flex-col">
                                <div class="mb-4">
                                    <h3 class="text-2xl font-bold text-white group-hover:text-amber-500 transition tracking-tight">{{ $car->marque }} {{ $car->modele }}</h3>
                                    <div class="flex items-center gap-2 mt-2 text-slate-500 text-sm font-medium">
                                        <div class="flex items-center gap-1.5 px-2 py-0.5 bg-white/5 rounded-md border border-white/5 uppercase tracking-tighter text-[10px]">{{ $car->annee }}</div>
                                        <span class="w-1 h-1 bg-slate-700 rounded-full"></span>
                                        <span>{{ $car->ville_origine }}, {{ $car->pays_origine }}</span>
                                    </div>
                                </div>
                                
                                <div class="grid grid-cols-2 gap-4 pb-6 mt-4 border-b border-white/5">
                                    <div class="flex items-center gap-2.5 text-xs font-medium text-slate-400">
                                        <div class="p-2 bg-slate-900 rounded-lg"><i data-lucide="gauge-circle" class="w-4 h-4 text-amber-500"></i></div>
                                        {{ number_format($car->kilometrage, 0, ',', ' ') }} km
                                    </div>
                                    <div class="flex items-center gap-2.5 text-xs font-medium text-slate-400">
                                        <div class="p-2 bg-slate-900 rounded-lg"><i data-lucide="fuel" class="w-4 h-4 text-amber-500"></i></div>
                                        {{ $car->carburant }}
                                    </div>
                                    <div class="flex items-center gap-2.5 text-xs font-medium text-slate-400">
                                        <div class="p-2 bg-slate-900 rounded-lg"><i data-lucide="settings-2" class="w-4 h-4 text-amber-500"></i></div>
                                        {{ $car->transmission }}
                                    </div>
                                    <div class="flex items-center gap-2.5 text-xs font-medium text-slate-400">
                                        <div class="p-2 bg-slate-900 rounded-lg"><i data-lucide="ship" class="w-4 h-4 text-amber-500"></i></div>
                                        {{ $car->portRecommande->nom ?? 'Port Togo' }}
                                    </div>
                                </div>

                                <div class="mt-auto pt-6 flex gap-3">
                                    <a href="{{ route('cars.show', $car->id) }}" class="flex-grow flex items-center justify-center py-4 text-sm font-bold transition border border-slate-800 bg-slate-900 text-white rounded-xl hover:bg-slate-800 hover:border-slate-700">
                                        Détails
                                    </a>
                    ...
                    @empty
                        <div class="col-span-2 py-32 text-center rounded-[2rem] border-2 border-dashed border-slate-900">
                            <i data-lucide="car-front" class="w-16 h-16 mx-auto mb-4 text-slate-800"></i>
                            <h3 class="text-xl font-bold text-white">Oups ! Aucun trésor ici.</h3>
                            <p class="text-slate-500 mt-2">Essayez de modifier vos filtres pour trouver votre bonheur.</p>
