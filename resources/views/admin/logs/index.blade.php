...
<div class="space-y-8">
 ...
 <div class="space-y-4">
  <div>
   <h1 class="text-xl font-semibold text-slate-900 dark:text-white tracking-normal uppercase amber-500 4 -offset-8 transition-colors">Audit Trail</h1>
   <p class="text-slate-500 dark:text-slate-400 font-medium mt-2 uppercase tracking-wide text-[10px] transition-colors">Journalisation granulaire des mutations et authentifications</p>
  </div>
  ...
  <div class="bg-slate-900 border border-slate-800 rounded-t-xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
   ...
   <form method="GET" class="flex flex-wrap gap-2 items-center">
    <div class="relative group">
     <input type="text"name="search"value="{{ request('search') }}"placeholder="grep 'keyword'..." class="bg-slate-900 text-emerald-500 font-mono text-xs border border-slate-800 rounded px-3 py-1.5 w-64 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/50 transition-all placeholder:text-slate-700">
    </div>
    <input type="date"name="date_start"value="{{ request('date_start') }}" class="bg-slate-900 text-slate-400 font-mono text-xs border border-slate-800 rounded px-2 py-1.5 focus:outline-none focus:border-slate-600">
    <span class="text-slate-600 font-mono text-xs">-</span>
    <input type="date"name="date_end"value="{{ request('date_end') }}" class="bg-slate-900 text-slate-400 font-mono text-xs border border-slate-800 rounded px-2 py-1.5 focus:outline-none focus:border-slate-600">
  ...
  <div class="bg-slate-900 border border-slate-800 rounded-b-xl overflow-hidden shadow-sm font-mono text-[11px] md:text-xs">
...
<div id="showLogModal" class="fixed inset-0 z-[100] hidden overflow-y-auto">
 <div class="fixed inset-0 bg-white dark:bg-slate-900/95 2xl transition-colors" onclick="closeModal('showLogModal')"></div>
 <div class="relative min-h-screen flex items-center justify-center p-4">
  <div class="relative bg-slate-900 border border-slate-800 w-full max-w-4xl shadow-sm rounded-xl overflow-hidden font-mono text-sm">
    ...
    <div class="grid grid-cols-2 gap-6">
     <div>
      <div class="text-[10px] text-slate-500 uppercase tracking-wide mb-1">OPERATOR</div>
      <div id="show_log_user" class="text-emerald-500 font-medium"></div>
     </div>
     <div>
      <div class="text-[10px] text-slate-500 uppercase tracking-wide mb-1">TIMESTAMP</div>
      <div id="show_log_date" class="text-blue-400"></div>
     </div>
    </div>

    <div class="p-4 bg-slate-900 rounded border border-slate-800">
     <div class="text-[10px] text-slate-500 uppercase tracking-wide mb-2">TARGET ENTITY</div>
    ...
    <div class="p-4 bg-slate-900 rounded border border-slate-800">
     <div class="text-[10px] text-slate-500 uppercase tracking-wide mb-2">TECHNICAL_DATA (JSON)</div>
    ...
    <div class="pt-4 text-center">
     <button onclick="closeModal('showLogModal')" class="px-6 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded text-xs font-mono uppercase transition-colors">
      [ CLOSE_REPORT ]
     </button>
    </div>
    </div>
