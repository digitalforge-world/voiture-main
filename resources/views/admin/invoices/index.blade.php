...
<div class="space-y-8">
 <div class="flex flex-col gap-6 md:flex-row md:items-center md:justify-between px-2">
  <div>
   <h1 class="text-xl font-semibold text-slate-900 dark:text-white tracking-normal uppercase amber-500 4 -offset-[-2px]">Factures PDF</h1>
   <p class="text-slate-500 dark:text-slate-400 font-medium mt-2 uppercase tracking-wide text-[10px]">Gérez vos factures clients</p>
  </div>
  <a href="{{ route('admin.invoices.create') }}" class="inline-flex items-center gap-3 px-6 py-3 bg-amber-500 text-slate-950 font-semibold uppercase tracking-wide text-[10px] rounded-xl hover:bg-amber-400 transition shadow-sm shadow-amber-900/20">
 ...
 <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4 px-2">
  @foreach(['draft' => ['Brouillons', 'file-edit', 'slate'], 'sent' => ['Envoyées', 'send', 'blue'], 'paid' => ['Payées', 'check-circle', 'emerald'], 'cancelled' => ['Annulées', 'x-circle', 'rose']] as $key => $info)
   <div class="group p-5 border bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-900 rounded-2xl hover:border-{{ $info[2] }}-500/30 transition shadow-lg dark:shadow-sm relative overflow-hidden">
    ...
    <div class="text-xl font-semibold text-slate-900 dark:text-white tracking-normal">{{ $invoices->where('status', $key)->count() }}</div>
    <div class="text-[10px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mt-2">{{ $info[0] }}</div>
   </div>
  @endforeach
 </div>

 <div class="border overflow-hidden bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-900 rounded-3xl shadow-sm dark:shadow-sm">
  <table class="w-full text-left">
   <thead class="bg-slate-50 dark:bg-slate-900 border-b border-slate-100 dark:border-white/5">
    <tr>
     <th class="px-6 py-4 text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Numéro</th>
     <th class="px-6 py-4 text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Client</th>
     <th class="px-6 py-4 text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Montant</th>
     <th class="px-6 py-4 text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Échéance</th>
    ...
    <tr class="group hover:bg-slate-50 dark:hover:bg-white/[0.02] transition duration-300">
     <td class="px-6 py-4">
      <div class="flex items-center gap-3">
       ...
       <div>
        <div class="text-sm font-semibold text-slate-900 dark:text-white">{{ $invoice->invoice_number }}</div>
        <div class="text-[9px] font-medium text-slate-400 dark:text-slate-600 mt-1 uppercase">{{ $invoice->created_at->format('d M Y') }}</div>
       </div>
      </div>
     </td>
     <td class="px-6 py-4">
      <div class="text-sm font-medium text-slate-900 dark:text-white">{{ $invoice->user->prenom }} {{ $invoice->user->nom }}</div>
      <div class="text-[9px] text-slate-400 dark:text-slate-600">{{ $invoice->user->email }}</div>
     </td>
     <td class="px-6 py-4">
      <div class="text-lg font-semibold text-slate-900 dark:text-white">{{ number_format($invoice->amount_total, 2, ',', ' ') }} <span class="text-xs text-amber-500">FCFA</span></div>
     </td>
     <td class="px-6 py-4">
     ...
     <td class="px-6 py-4">
      ...
      <span class="inline-flex items-center px-3 py-1 text-[9px] font-semibold uppercase tracking-wide border rounded-full bg-{{ $s[1] }}-500/10 text-{{ $s[1] }}-500 border-{{ $s[1] }}-500/20">{{ $s[0] }}</span>
     </td>
