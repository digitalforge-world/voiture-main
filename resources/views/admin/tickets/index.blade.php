...
<div class="space-y-8">
 <div class="flex flex-col gap-6 md:flex-row md:items-center md:justify-between px-2">
  <div>
   <h1 class="text-xl font-semibold text-slate-900 dark:text-white tracking-normal uppercase amber-500 4 -offset-[-2px]">Support Client</h1>
   <p class="text-slate-500 dark:text-slate-400 font-medium mt-2 uppercase tracking-wide text-[10px]">Gérez les demandes d'assistance</p>
  </div>
  <a href="{{ route('admin.tickets.create') }}" class="inline-flex items-center gap-3 px-6 py-3 bg-amber-500 text-slate-950 font-semibold uppercase tracking-wide text-[10px] rounded-xl hover:bg-amber-400 transition shadow-sm shadow-amber-900/20">
 ...
 <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4 px-2">
  @foreach(['open' => ['Ouverts', 'circle-dot', 'amber'], 'answered' => ['Répondus', 'check-circle', 'emerald'], 'resolved' => ['Résolus', 'check-check', 'blue'], 'urgent' => ['Urgents', 'alert-circle', 'rose']] as $key => $info)
   <div class="group p-5 border bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-900 rounded-2xl hover:border-{{ $info[2] }}-500/30 transition shadow-lg dark:shadow-sm relative overflow-hidden">
    ...
    <div class="text-xl font-semibold text-slate-900 dark:text-white tracking-normal">{{ $key === 'urgent' ? $tickets->where('priority', 'urgent')->count() : $tickets->where('status', $key)->count() }}</div>
 ...
 <div class="border overflow-hidden bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-900 rounded-3xl shadow-sm dark:shadow-sm">
  <table class="w-full text-left">
   <thead class="bg-slate-50 dark:bg-slate-900 border-b border-slate-100 dark:border-white/5">
    <tr>
     <th class="px-6 py-4 text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Ticket</th>
     <th class="px-6 py-4 text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Client</th>
     <th class="px-6 py-4 text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Priorité</th>
    ...
    <tr class="group hover:bg-slate-50 dark:hover:bg-white/[0.02] transition duration-300">
     <td class="px-6 py-4">
      <div class="flex items-center gap-3">
       ...
       <div>
        <div class="text-sm font-semibold text-slate-900 dark:text-white">#{{ $ticket->id }} - {{ $ticket->subject }}</div>
        <div class="text-[9px] font-medium text-slate-400 dark:text-slate-600 mt-1 uppercase">{{ $ticket->created_at->diffForHumans() }}</div>
       </div>
      </div>
     </td>
     <td class="px-6 py-4">
      <div class="text-sm font-medium text-slate-900 dark:text-white">{{ $ticket->user->prenom }} {{ $ticket->user->nom }}</div>
     ...
     <td class="px-6 py-4">
      ...
      <span class="inline-flex items-center px-3 py-1 text-[9px] font-semibold uppercase tracking-wide border rounded-full bg-{{ $p[1] }}-500/10 text-{{ $p[1] }}-500 border-{{ $p[1] }}-500/20">{{ $p[0] }}</span>
     </td>
     <td class="px-6 py-4">
      ...
      <span class="inline-flex items-center px-3 py-1 text-[9px] font-semibold uppercase tracking-wide border rounded-full bg-{{ $s[1] }}-500/10 text-{{ $s[1] }}-500 border-{{ $s[1] }}-500/20">
