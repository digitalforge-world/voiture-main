            ...
            <form id="editCarForm" method="POST" enctype="multipart/form-data" class="space-y-12 relative max-h-[70vh] overflow-y-auto px-4 custom-scrollbar">
                ...
                <div class="space-y-6">
                    ...
                    <div class="pt-6 border-t border-slate-100 dark:border-white/5">
                        ...
                        <div id="existing_media_container" class="flex flex-wrap gap-4 empty:hidden"></div>
                    </div>
                </div>
...
<script>
    ...
    function openEditCarModal(car) {
        ...
        // Reset and Load Existing Media
        const mediaContainer = document.getElementById('existing_media_container');
        mediaContainer.innerHTML = '';
        
        if (car.photo_principale) {
            addMediaThumb(mediaContainer, 'photo', car.photo_principale, 'principale', 'car');
        }
        
        if (car.photos) {
            car.photos.forEach(p => {
                if (p.url !== car.photo_principale) {
                    addMediaThumb(mediaContainer, 'photo', p.url, p.id, 'car');
                }
            });
        }
        
        if (car.videos) {
            car.videos.forEach(v => {
                addMediaThumb(mediaContainer, 'video', v.url, v.id, 'car');
            });
        }
